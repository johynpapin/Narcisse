...
func commandSet(s *discordgo.Session, m *discordgo.MessageCreate, args []string) error {
	...
	case "set":
		...
		s.ChannelMessageSend(m.ChannelID, "Votre fuseau horaire est désormais **"+args[0]+"**, dans ce fuseau horaire, il est **"+now.Format("15:04:05")+"**.")
	case "get":
		if len(args) < 1 {
			s.ChannelMessageSend(m.ChannelID, "Il manque le fuseau horaire. Utilisez la commande `help time` pour accéder à l’aide.")
		}
		...
		s.ChannelMessageSend(m.ChannelID, "Dans ce fuseau horaire, il est **"+now.Format("15:04:05")+"**.")
	default:
		if len(m.Mentions) == 0 {
			...
			for _, user := range m.Mentions {
				...
				if timezone == "" {
					...
					s.ChannelMessageSend(m.ChannelID, "Dans le fuseau horaire de **"+user.Username+"**, il est **"+now.Format("15:04:05")+"**.")
